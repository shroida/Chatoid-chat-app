...
import 'package:chatoid/zRefactor/features/login/repository/login_repo.dart';
...
class LoginRepoImpl with LoginRepo {
  ...
  Future<void> fillCurrentUserDataByEmail(
    ...
    try {
      ...
      final response = await supabase
          ...
          .eq('email', email)
          .single();
      loginCubit.currentUser.user_id = response['user_id'] as int;
  ...
  Future<void> saveOneSignalPlayerId(BuildContext context) async {
    try {
      ...
      if (playerId == null) {
        var status = await OneSignal.User.pushSubscription;
      ...
      if (playerId != null) {
        await Supabase.instance.client.from('user_profiles').upsert({
          'user_id': loginCubit.currentUser.user_id,
          'player_id': playerId,
          'email':
              loginCubit.currentUser.email, // Make sure to include email here
        });
      } else {}
    } catch (e) {}
  }
